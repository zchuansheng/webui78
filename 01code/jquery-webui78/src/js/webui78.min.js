...
////////////////////////////////////////////////////////////////////////////////
//页面加载前等待效果
document.write("<div class='ui-mask' ></div><div class='ui-mask-msg'  ><div style='z-index: 100;'>页面正在加载......</div></div>");
$(window).bind("load",function(){$(document).unmask();})
if($.browser==null) {
	$.browser={};
	$.browser.msie = /msie/.test(navigator.userAgent.toLowerCase());
}
if ($.browser.msie) {
	window.onbeforeprint=function(){$(document).unmask();};
}
/*增加自动传递表单标题功能*/
...
(function($){
	$.webUtil={
		...
		submit:function(url,op){
			...
			var sbFlag=frm1.data("submitting");
		...
		toSubmit:function(frm1,url,op){
			...
			if(frm1.data("submitting")) return false;//如果正在提交标识为true，则直接返回
			frm1.data("submitting",true);//否则，设置正在提交标识为true
			var maskobj=$(op.mask);
			if(op.ismask) {//如果需要遮罩则，设置遮罩效果
				maskobj.setmask({msg:$.webUtil.setting.messageOnLoading,caller:op.btn});
			}
			if(op.ischeck &&(!frm1.formIsValid())) {//如果需要检查表单，且检查失败 取消遮罩，提交标识
			...
			function ok(){
				if(op.trgt!=null) frm1.attr("target",op.trgt);
				if(op.trgt!=null &&  op.trgt!=""){//如果指定了trgt则，取消遮罩，取消提交标识
					 frm1.data("submitting",false);
					 if(op.ismask) maskobj.unmask();
				}
				if(op.ajax==null){//如果非ajax提交，则调用表单的submit
					   	...
					   	bo.data("btn_setting",op);
							...
							if(op.enableIcon) {//如果启用按钮图标，则追加按钮图标的元素
								...
								if(op.icon!=null){ic.addClass(op.icon);}
									...
									var icc=$.webUtil.setting.btn_icons[txt.substring(0,2)];
 		...
 		$.webDialogUtil={
			...
			openedCount:0,
 			createDlg:function(op){
					...
					dialog.css({top:($(window).height()-parseInt(op.top))/2,left:($(window).width()-parseInt(op.left))/2,
			...
			return this.each(function(){
				...
				var dialog={
					...
					show:function(op){
						...
						var toLeft=($(window).width()-toWidth)/2;
						var toTop=($(window).height()-toHeight)/3+$.webDialogUtil.openedCount*30;
						if(toTop<5) toTop=5;
